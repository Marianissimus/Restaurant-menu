...
describe('menu test', () => {
  ...
  it('can show the correct menu items by default', () => {
    ...
    cy.get('.menu-list li:first .menu-preview  h1 .name').should('have.text', 'Asparagus BBQ')
  ...
  it('can display no of items in shopping cart preview', () => {
    ...
    cy.get('[data-cy="Asparagus-container"] .add-to-cart').click()
  ...
  it('can add and remove items to cart and calculate total', () => {
    ...
    for(let i = 0; i < 5; i++){
      cy.get('[data-cy="Brownie-container"] .add-to-cart').click()
    }
    cy.get('#how-many-items span').should('have.text', '12')
    cy.get('.shopping-cart').click()
    cy.get('#totalValue').should('have.text', '800 lei')
    cy.get('[data-cy="Asparagus-tr"]  .remove-item-btn').click()
    cy.get('#totalValue').should('have.text', '700 lei')
    cy.get('[data-cy="Brownie-tr"]  .remove-item-btn').click()
  ...
  it('can group items inside shopping cart and show subtotals', () => {
    ...
    cy.get('[data-cy="Asparagus-container"] .add-to-cart').click()
    cy.get('.shopping-cart').click()
    cy.get('[data-cy="Asparagus-tr"] .item-times').should('have.text', '3')
    cy.get('[data-cy="Asparagus-tr"] .item-unit-price').should('have.text', '100')
    cy.get('[data-cy="Asparagus-tr"] .subtotal').should('have.text', '300')
    cy.get('[data-cy="Brownie-tr"] .item-times').should('have.text', '2')
    cy.get('[data-cy="Brownie-tr"] .item-unit-price').should('have.text', '20')
    cy.get('[data-cy="Brownie-tr"] .subtotal').should('have.text', '40')
  })
  ...
  it('can show the correct message when no items in shopping cart', () => {
    ...
    cy.get('.back-to-menu-btn:first').click()
    cy.get('[data-cy="Asparagus-container"]  .add-to-cart').click({multiple: true})
    cy.get('.shopping-cart').click()
